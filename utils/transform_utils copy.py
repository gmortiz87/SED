...
def transformar_fuente(nombre_fuente: str, anio: str = None):
    ...
    if not df_metas.empty:
        df_metas_rel = df_metas.merge(
            df_proy_rel[["Hoja", "ID_Proyecto"]] if "ID_Proyecto" in df_proy_rel.columns else df_proy_rel,
            on=["Hoja"], how="left"
            # df_proy_rel[["Hoja", "Nombre_Proyecto", "ID_Proyecto"]] if "ID_Proyecto" in df_proy_rel.columns else df_proy_rel,
            # on=["Hoja", "Nombre_Proyecto"], how="left"
        )
        df_metas_rel = df_metas_rel.merge(df_rel, left_on="Hoja", right_on="PROYECTOS", how="left")
