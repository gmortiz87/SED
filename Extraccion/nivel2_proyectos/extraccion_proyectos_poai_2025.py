...
def run(fuente="POAI_2025"):
    ...
    df_proyectos, df_actividades, df_metas= procesar_fuente_proyectos(
        ...
        fuente,
        hojas,
        f"Registro_Proyectos_Actividades_POAI_2025.xlsx"
    )

    end = time.time()
    
    print(f"[INFO] {len(df_proyectos)} proyectos, {len(df_actividades)} actividades y {len(df_metas)} metas exportadas")

    print("\n[OK] Primeros registros POAI 2025:")
